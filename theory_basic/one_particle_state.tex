...
\begin{equation}\label{total_lorentz_group}
...
\end{equation}
其中 $D^{(j)}_{\bar{\sigma}\sigma}$ 是 $SU(2)$ 自旋 $j$  旋转矩阵，$W(\Lambda,p) = L^{-1}(\Lambda p)\;\;\Lambda\;\;L(p)$ 是 Wigner 旋转。关键的物理图像是：粒子的动量在 Lorentz 变换下改变（$\mathbf{p}\to\Lambda\mathbf{p}$），而自旋态在 $2j+1$ 维内部空间中进行旋转混合——这种混合由 Wigner 旋转通过 $SU(2)$ 表示矩阵来实现。
\newpage
