...
\begin{subequations}\label{su2+su2}
    \begin{align}
        &\mathcal{A}_i=\frac{1}{2}(J_i-iK_i)\\
        &\mathcal{B}_i=\frac{1}{2}(J_i+iK_i)
    \end{align}
\end{subequations}
这俩个算符的对易关系是：
\begin{subequations}
    \begin{align}
        &[\mathcal{A}_i,\mathcal{A}_j]=i\epsilon_{ijk} \mathcal{A}_{k}\\
        &[\mathcal{B}_i,\mathcal{B}_j]=i\epsilon_{ijk} \mathcal{B}_{k}\\
...
\begin{subequations}
\begin{align}
    \braket{a',b'|\boldsymbol{\mathcal{A}}|a,b}
    &= 
...
\end{align}
\begin{align}
    \braket{a',b'|\boldsymbol{\mathcal{B}}|a,b}
    &=
...
\begin{equation}
    \boldsymbol{\mathcal{A}} = \frac{1}{2}(\boldsymbol{J}+i\boldsymbol{K}),\quad \boldsymbol{\mathcal{B}} = \frac{1}{2}(\boldsymbol{J}-i\boldsymbol{K})
\end{equation}
其中 $\boldsymbol{J}$ 是角动量（旋转生成元），$\boldsymbol{K}$ 是 Boost 生成元。由于 $\boldsymbol{J}$ 厄米而 $\boldsymbol{K}$ 必须反厄米（这正是非紧致性的体现），我们有：
\begin{itemize}
    \item 旋转生成元 $\boldsymbol{J} = \boldsymbol{\mathcal{A}}+\boldsymbol{\mathcal{B}}$ 是厄米的 $\Rightarrow$ 旋转矩阵 $e^{i\boldsymbol{\theta}\cdot\boldsymbol{J}}$ 是幺正的。
    \item Boost 生成元 $\boldsymbol{K} = -i(\boldsymbol{\mathcal{A}}-\boldsymbol{\mathcal{B}})$ 是\textbf{反厄米}的 $\Rightarrow$ Boost 矩阵 $e^{-i\boldsymbol{\phi}\cdot\boldsymbol{K}} = e^{-\boldsymbol{\phi}\cdot(\boldsymbol{\mathcal{A}}-\boldsymbol{\mathcal{B}})}$ \textbf{不是幺正的}。
\end{itemize}
...
\begin{equation}
    \mathcal{P}:\quad \boldsymbol{\mathcal{A}}=\frac{1}{2}(\boldsymbol{J}+i\boldsymbol{K}) \longleftrightarrow \frac{1}{2}(\boldsymbol{J}-i\boldsymbol{K})=\boldsymbol{\mathcal{B}}
\end{equation}
...
\end{align}
这正是狄拉克旋量必须是四分量的深层含义，也是电磁场张量 $F_{\mu\nu}$ 必须同时包含电场 $\mathbf{E}$（极矢量）和磁场 $\boldsymbol{\mathcal{B}}$（赝矢量）的根本原因。
