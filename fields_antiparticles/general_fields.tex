...
\begin{subequations}\label{su2+su2}
    \begin{align}
        &\boldsymbol{J}^{(A)}_i=\frac{1}{2}(J_i-iK_i)\\
        &\boldsymbol{J}^{(B)}_i=\frac{1}{2}(J_i+iK_i)
    \end{align}
\end{subequations}
这俩个算符的对易关系是：
\begin{subequations}
    \begin{align}
        &[J^{(A)}_i,J^{(A)}_j]=i\epsilon_{ijk} J^{(A)}_{k}\\
        &[J^{(B)}_i,J^{(B)}]=i\epsilon_{ijk} J^{(B)}_{k}\\
        &[J^{(A)}_i,J^{(B)}_j]=0
    \end{align}
\end{subequations}
...
\begin{equation}
    \boldsymbol{J}^{(A)} = \frac{1}{2}(\boldsymbol{J}+i\boldsymbol{K}),\quad \boldsymbol{J}^{(B)}= \frac{1}{2}(\boldsymbol{J}-i\boldsymbol{K})
\end{equation}
其中 $\boldsymbol{J}$ 是角动量（旋转生成元），$\boldsymbol{K}$ 是 Boost 生成元。由于 $\boldsymbol{J}$ 厄米而 $\boldsymbol{K}$ 必须反厄米（这正是非紧致性的体现），我们有：
\begin{itemize}
    \item 旋转生成元 $\boldsymbol{J} = \boldsymbol{J}^{(A)}+\boldsymbol{J}^{(B)}$ 是厄米的 $\Rightarrow$ 旋转矩阵 $e^{i\boldsymbol{\theta}\cdot\boldsymbol{J}}$ 是幺正的。
    \item Boost 生成元 $\boldsymbol{K} = -i(\boldsymbol{J}^{(A)}-\boldsymbol{J}^{(b)})$ 是\textbf{反厄米}的 $\Rightarrow$ Boost 矩阵 $e^{-i\boldsymbol{\phi}\cdot\boldsymbol{K}} = e^{-\boldsymbol{\phi}\cdot(\boldsymbol{J}^{(A)}-\boldsymbol{J}^{(B)})}$ \textbf{不是幺正的}。
\end{itemize}
...
\begin{equation}
    \mathcal{P}:\quad \boldsymbol{J}^{(A)}=\frac{1}{2}(\boldsymbol{J}+i\boldsymbol{K}) \longleftrightarrow \frac{1}{2}(\boldsymbol{J}-i\boldsymbol{K})=\boldsymbol{J}^{(B)}
\end{equation}
