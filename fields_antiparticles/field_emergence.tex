...
\begin{flalign}
    ...
    &D_{\sigma \bar{\sigma}}^{j_n*}\left\{W^{-1}(\Lambda,p)\right\}a^\dagger(\Lambda\boldsymbol{p},\bar{\sigma},n)\\&=\sum_{\bar{\ell}}\sum_{\sigma}\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1})u_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\sigma,n) a^\dagger(\Lambda \boldsymbol{p},\sigma,n)
\end{aligned}&
\end{flalign}
\label{u_1}
\begin{flalign}
    &\begin{aligned}\text{
令 $\bar{\sigma}\rightleftarrows \sigma$所以：}
