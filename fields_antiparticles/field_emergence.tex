...
场论的限制\begin{enumerate}
    ...
    \item 集团分解原理表明：若用产生算子 $a(\boldsymbol{p},\sigma,n)$与湮灭算子$a^\dagger(\boldsymbol{p},\sigma,n)$构造$S$矩阵的连接部分$S^\mathbb{C}\thicksim \mathscr{H}$，则对应的位置空间产生场与湮灭场必须为
            \begin{subequations}
                   ...
                   \end{align}
            \end{subequations}
            对系数$u_\ell(x;\boldsymbol{p},\sigma,n)$和$v_\ell(x;\boldsymbol{p},\sigma,n)$进行选择，可以让$\Psi^\pm$的Lorentz变换\footnote{原则上，对于不同的场，有不同的变换矩阵 $D^{\pm}$，我们看到 $D$-矩阵构成了齐次 Lorentz 群的一个表示：
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
所以对于场的Lorentz变换：即可以利用产生(湮灭)算符的\text{Poincar\'e}变换进行表示，也可以利用产生(湮灭)场的Lorentz变换进行表示，简写就是：
\begin{equation}
...
\[\begin{aligned}
    ...
    =\sum_{\bar{\ell}}\sum_{\bar{\sigma},n}\int d^3\Lambda p\,\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1}) u_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n) a^\dagger(\Lambda \boldsymbol{p},\bar{\sigma},n)
\end{aligned}\]
左右两边相等$L.H.S=R.H.S$所以,
\[    \begin{aligned}
    &\sum_{\sigma\bar{\sigma}}u_{\ell}(x;\boldsymbol{p},\sigma,n)\,e^{-i(\Lambda p)^\mu a_\mu}\sqrt{\frac{p^0}{(\Lambda p)^0}}
    D_{\sigma \bar{\sigma}}^{j_n*}\left\{W^{-1}(\Lambda,p)\right\}a^\dagger(\Lambda\boldsymbol{p},\bar{\sigma},n)
    \\=&\sum_{\bar{\ell}}\sum_{\bar{\sigma}}\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1})u_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n) a^\dagger(\Lambda \boldsymbol{p},\bar{\sigma},n)
\end{aligned}\]
\[\implies
    \begin{aligned}
            \sum_{\bar{\sigma}}u_{\ell}(x;\boldsymbol{p},\sigma,n)\,e^{-i(\Lambda p)^\mu a_\mu}\sqrt{\frac{p^0}{(\Lambda p)^0}}D_{\sigma \bar{\sigma}}^{j_n*}\left\{W^{-1}(\Lambda,p)\right\}=\sum_{\bar{\ell}}\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1})u_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n)
    \end{aligned}\]

最后写为，
\begin{equation}
    \begin{aligned}
            \sum_{\bar{\sigma}}u_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n)D_{\bar{\sigma}\sigma }^{j_n*}\left\{W(\Lambda,p)\right\}=\sqrt{\frac{p^0}{(\Lambda p)^0}}\,e^{-i(\Lambda p)^\mu a_\mu}\sum_{\bar{\ell}}\mathscr{D}_{\bar{\ell}\ell}(\Lambda)u_{\ell}(x;\boldsymbol{p},\sigma,n)
    \end{aligned}
\end{equation}
\centerline{\text{湮灭场的系数}}
\[\begin{aligned}
    L.H.S=&\;U_0(\Lambda,a)\Psi^{-}_\ell(x)U_0^\dagger(\Lambda,a)\\
    =&\sum_{\sigma,n}\int d^3p\, v_{\ell}(x;\boldsymbol{p},\sigma,n) U_0(\Lambda,a)a^\dagger(\boldsymbol{p},\sigma,n)U_0^\dagger(\Lambda,a)\\
    =&\sum_{\sigma,n}\int d^3p\, v_{\ell}(x;\boldsymbol{p},\sigma,n)\,e^{+i(\Lambda p)^\mu a_\mu}\sqrt{\frac{(\Lambda p)^0}{p^0}}\sum_{\bar{\sigma}}D_{\sigma \bar{\sigma}}^{j_n}\left\{W^{-1}(\Lambda,p)\right\}a^\dagger(\Lambda\boldsymbol{p},\bar{\sigma},n)\\
    &\xrightarrow{d^3p\to \frac{d^3(\Lambda p)}{(\Lambda p)^0}p^0}\sum_{\sigma\bar{\sigma},n}\int d^3\Lambda p\, v_{\ell}(x;\boldsymbol{p},\sigma,n)\,e^{+i(\Lambda p)^\mu a_\mu}\sqrt{\frac{p^0}{(\Lambda p)^0}}\,D_{\sigma \bar{\sigma}}^{j_n}\left\{W^{-1}(\Lambda,p)\right\} a^\dagger(\Lambda\boldsymbol{p},\bar{\sigma},n)
\end{aligned}\]
\[\begin{aligned}
    R.H.S=\sum_{\bar{\ell}}\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1})\Psi_{\bar{\ell}}^+(\Lambda x+a)
    =\sum_{\bar{\ell}}\sum_{\bar{\sigma},n}\int d^3\Lambda p\,\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1}) v_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n) a^\dagger(\Lambda \boldsymbol{p},\bar{\sigma},n)
\end{aligned}\]
左右两边相等$L.H.S=R.H.S$所以,
\[    \begin{aligned}
    &\sum_{\sigma\bar{\sigma}}v_{\ell}(x;\boldsymbol{p},\sigma,n)\,e^{+i(\Lambda p)^\mu a_\mu}\sqrt{\frac{p^0}{(\Lambda p)^0}}
    D_{\sigma \bar{\sigma}}^{j_n}\left\{W^{-1}(\Lambda,p)\right\}a^\dagger(\Lambda\boldsymbol{p},\bar{\sigma},n)
    \\=&\sum_{\bar{\ell}}\sum_{\bar{\sigma}}\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1})v_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n) a^\dagger(\Lambda \boldsymbol{p},\bar{\sigma},n)
\end{aligned}\]
\[\implies
    \begin{aligned}
            \sum_{\bar{\sigma}}v_{\ell}(x;\boldsymbol{p},\sigma,n)\,e^{i(\Lambda p)^\mu a_\mu}\sqrt{\frac{p^0}{(\Lambda p)^0}}D_{\sigma \bar{\sigma}}^{j_n}\left\{W^{-1}(\Lambda,p)\right\}=\sum_{\bar{\ell}}\mathscr{D}_{\ell\bar{\ell}}(\Lambda^{-1})v_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n)
    \end{aligned}\]
最后写为，
\begin{equation}
    \begin{aligned}
            \sum_{\bar{\sigma}}v_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n)D_{\bar{\sigma}\sigma }^{j_n}\left\{W(\Lambda,p)\right\}=\sqrt{\frac{p^0}{(\Lambda p)^0}}\,e^{+i(\Lambda p)^\mu a_\mu}\sum_{\bar{\ell}}\mathscr{D}_{\bar{\ell}\ell}(\Lambda)v_{\ell}(x;\boldsymbol{p},\sigma,n)
    \end{aligned}
\end{equation}
\hrule
\vspace{2ex}
将产生场和湮灭场的系数变换公式写在一起，它们是：
\begin{subequations}
    \begin{align}
        &\sum_{\bar{\sigma}}u_{\bar{\ell}}(\Lambda x+a;\Lambda \boldsymbol{p},\bar{\sigma},n)D_{\bar{\sigma}\sigma }^{j_n*}\left\{W(\Lambda,p)\right\}=\sqrt{\frac{p^0}{(\Lambda p)^0}}\,e^{-i(\Lambda p)^\mu a_\mu}\sum_{\bar{\ell}}\mathscr{D}_{\bar{\ell}\ell}(\Lambda)u_{\ell}(x;\boldsymbol{p},\sigma,n)
        \\
    ...
    \end{align}
\end{subequations}
\color{blue}到现在为止，一直在回避一个问题：构建局域场算符时：“粒子”对应 $\text{Poincar\'e}$ 群，而“场”对应Lorentz群
\begin{itemize}
...
\begin{subequations}
    \begin{align}
        &\sum_{\bar{\sigma}}u_{\bar{\ell}}(\Lambda \boldsymbol{p},\bar{\sigma},n)D_{\bar{\sigma}\sigma }^{j_n*}\left\{W(\Lambda,p)\right\}=\sqrt{\frac{p^0}{(\Lambda p)^0}}\sum_{\bar{\ell}}\mathscr{D}_{\bar{\ell}\ell}(\Lambda)u_{\ell}(\boldsymbol{p},\sigma,n)
        \\
        &\sum_{\bar{\sigma}}v_{\bar{\ell}}(\Lambda \boldsymbol{p},\bar{\sigma},n)D_{\bar{\sigma}\sigma }^{j_n}\left\{W(\Lambda,p)\right\}=\sqrt{\frac{p^0}{(\Lambda p)^0}}\sum_{\bar{\ell}}\mathscr{D}_{\bar{\ell}\ell}(\Lambda)v_{\ell}(\boldsymbol{p},\sigma,n)
    \end{align}
\end{subequations}
正常接下来应该讨论$Boost$和$Rotation$；但这里先停止，将这两种变换与系数矩阵的关系放在构造场之前.
\topictitle{验证集团分解}
回到集团分解中来，我们得到了场的一般形式，将产生场和湮灭场带入到之前构造的相互作用，并把位置积掉;
\begin{equation}\label{eq_Hint_general}
\begin{aligned}
    ...
    &g_{\ell'_1 \cdots \ell'_N, \, \ell_1 \cdots \ell_M}  v_{\ell'_1}(\mathbf{p}'_1 \sigma'_1 n'_1) \cdots v_{\ell'_N}(\mathbf{p}'_N \sigma'_N n'_N) u_{\ell_1}(\mathbf{p}_1 \sigma_1 n_1) \cdots u_{\ell_M}(\mathbf{p}_M \sigma_M n_M) \ .
\end{aligned}
\end{equation}
...
\end{itemize}
\normalcolor
这正是量子场论的核心，\textbf{集团分解}与\textbf{Lorentz不变性}，共同迫使相互作用密由局域的产生场和湮灭场来构建。
\topictitle{Lorentz不变性的要求：微观因果性}
...
\begin{align}
    ...
    \frac{1}{(2\pi)^3}\sum_{\sigma,n}\int d^3 \boldsymbol{p}\,v_\ell(\boldsymbol{p},\sigma,n)u_{\bar{\ell}}(\boldsymbol{p},\sigma,n)e^{ip_\mu(x-x')^\mu}
\end{align}
\end{subequations}
