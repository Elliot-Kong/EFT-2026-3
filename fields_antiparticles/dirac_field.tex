...
\end{center}
\vspace{0.5cm}
对于自旋 $j=1/2$ 的有质量粒子，在静止参考系（$\mathbf{p}=\mathbf{0}$，$p^0=m$）中，狄拉克方程 $(\gamma^\mu p_\mu - m)u=0$ 退化为：
\begin{equation}
...
\end{equation}
利用泡利矩阵的性质 $(\hat{\mathbf{p}}\cdot\boldsymbol{\sigma})^2 = I$ 以及双曲半角公式（$\cosh\phi = p^0/m$，$\sinh\phi = |\mathbf{p}|/m$）：
\begin{align}
    e^{+\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}} &= \cosh\frac{\phi}{2}\cdot I + \sinh\frac{\phi}{2}\cdot\hat{\mathbf{p}}\cdot\boldsymbol{\sigma} = \frac{(p^0+m)I + \mathbf{p}\cdot\boldsymbol{\sigma}}{\sqrt{2m(p^0+m)}} \\
    e^{-\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}} &= \cosh\frac{\phi}{2}\cdot I - \sinh\frac{\phi}{2}\cdot\hat{\mathbf{p}}\cdot\boldsymbol{\sigma} = \frac{(p^0+m)I - \mathbf{p}\cdot\boldsymbol{\sigma}}{\sqrt{2m(p^0+m)}}
\end{align}
将 Boost 矩阵作用于静止系旋量 $u(0,\sigma) = \sqrt{m}\binom{\xi_\sigma}{\xi_\sigma}$：
\begin{equation}
    u(\mathbf{p},\sigma) = S(L(p))\, u(0,\sigma) = \sqrt{m}\begin{pmatrix} e^{+\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}}\xi_\sigma \\ e^{-\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}}\xi_\sigma \end{pmatrix}
\end{equation}
四维记号下： $\sqrt{p\cdot\bar{\sigma}} \equiv e^{+\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}}\sqrt{m}$ 和 $\sqrt{p\cdot\sigma} \equiv e^{-\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}}\sqrt{m}$（其中 $p\cdot\sigma = p^0 I - \mathbf{p}\cdot\boldsymbol{\sigma}$，$p\cdot\bar{\sigma} = p^0 I + \mathbf{p}\cdot\boldsymbol{\sigma}$），则：
\begin{equation}
    \boxed{u(\mathbf{p},\sigma) = \begin{pmatrix} \sqrt{p\cdot\bar{\sigma}}\,\xi_\sigma \\ \sqrt{p\cdot\sigma}\,\xi_\sigma \end{pmatrix}}
\end{equation}
将 Boost 矩阵作用于 $v(0,\sigma) = \sqrt{m}\binom{\chi_\sigma}{-\chi_\sigma}$（注意下半部分的负号在 Boost 后被保留）：
\begin{equation}
    \boxed{v(\mathbf{p},\sigma) = \begin{pmatrix} \sqrt{p\cdot\bar{\sigma}}\,\chi_\sigma \\ -\sqrt{p\cdot\sigma}\,\chi_\sigma \end{pmatrix}}
\end{equation}
...
\begin{itemize}
    \item 左上块：$\sqrt{p\cdot\bar{\sigma}}\cdot I\cdot\sqrt{p\cdot\sigma} = \sqrt{(p\cdot\bar{\sigma})(p\cdot\sigma)}$。利用 $(p\cdot\bar{\sigma})(p\cdot\sigma) = (p^0)^2 - |\mathbf{p}|^2 = m^2$，得 $= mI$。
    \item 右上块：$\sqrt{p\cdot\bar{\sigma}}\cdot I\cdot\sqrt{p\cdot\bar{\sigma}} = p\cdot\bar{\sigma} = p^0 I + \mathbf{p}\cdot\boldsymbol{\sigma}$。
    \item 左下块：$\sqrt{p\cdot\sigma}\cdot I\cdot\sqrt{p\cdot\sigma} = p\cdot\sigma = p^0 I - \mathbf{p}\cdot\boldsymbol{\sigma}$。
    \item 右下块：$\sqrt{p\cdot\sigma}\cdot I\cdot\sqrt{p\cdot\bar{\sigma}} = mI$。
\end{itemize}
因此：
\begin{equation}
    \sum_\sigma u(\mathbf{p},\sigma)\bar{u}(\mathbf{p},\sigma) = \begin{pmatrix} mI & p\cdot\bar{\sigma} \\ p\cdot\sigma & mI \end{pmatrix} = \gamma^\mu p_\mu + m \equiv \slashed{p}+m
    \label{eq:pol_u}
\end{equation}
对于反粒子旋量 $v$，由于下半部分负号导致对角块反号：
\begin{equation}
   \sum_\sigma v(\mathbf{p},\sigma)\bar{v}(\mathbf{p},\sigma) = \slashed{p}-m
    \label{eq:pol_v}
\end{equation}
