...
在上一节对 Weyl 场的构造中，我们得出了一个深刻的结论：单独的左手 Weyl 场 $(1/2, 0)$ 在空间反演下无法保持封闭——宇称操作 $\mathcal{P}$ 使得 Boost 生成元符号翻转 $\mathbf{K} \to -\mathbf{K}$，将 $(1/2, 0)$ 的 Boost 矩阵 $e^{-\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}}$ 变成了 $(0, 1/2)$ 的 Boost 矩阵 $e^{+\frac{1}{2}\boldsymbol{\phi}\cdot\boldsymbol{\sigma}}$。

因此，如果大自然中存在具有空间反演对称性且有质量的自旋 $1/2$ 粒子（如电子），其数学描述必须同时包容左手和右手两种手征性。本节我们将详细搭建这一四分量旋量空间的数学结构，为下一节的量子化场论构造打下严密的基础。
\vspace{2ex}\par
在 Weyl 场中，$(1/2, 0)$ 表示的生成元为：
\begin{equation}
    \mathbf{J}^{(A)} = \frac{1}{2}(\mathbf{J} + i\mathbf{K}) = \frac{\boldsymbol{\sigma}}{2}, \quad \mathbf{J}^{(B)} = \frac{1}{2}(\mathbf{J} - i\mathbf{K}) = 0
\end{equation}
空间反演使 $\mathbf{K} \to -\mathbf{K}$，$\mathbf{J} \to \mathbf{J}$，因此：
\begin{equation}
    \mathcal{P}: \quad \mathbf{J}^{(A)} = \frac{1}{2}(\mathbf{J} + i\mathbf{K}) \longrightarrow \frac{1}{2}(\mathbf{J} - i\mathbf{K}) = \mathbf{J}^{(B)}
\end{equation}
...
\end{equation}
这便是四分量狄拉克旋量的来源。我们将四分量 Dirac 旋量 $\psi$ 构造为左右手 Weyl 旋量的列向量：
\begin{equation}
...
\begin{align}
    p_\mu \bar{\sigma}^\mu \psi_R &= (p^0 + \mathbf{p}\cdot\boldsymbol{\sigma}) \psi_R = 0 \\
    p_\mu \sigma^\mu \psi_L &= (p^0 - \mathbf{p}\cdot\boldsymbol{\sigma}) \psi_L = 0
\end{align}
其中 $\sigma^\mu = (I, \boldsymbol{\sigma})$，$\bar{\sigma}^\mu = (I, -\boldsymbol{\sigma})$。
当粒子具有静止质量 $m \neq 0$ 时，在静止参考系中（$\mathbf{p}=0, p^0=m$），洛伦兹群退化为空间旋转群 $SU(2)$。此时，左手 Weyl 旋量与右手 Weyl 旋量在纯旋转下变换完全相同，它们无法通过旋转来区分。因此，一个有质量的粒子在静止时不具有确定的手征性——左手态和右手态必须不可避免地通过质量 $m$ 发生耦合。
...
容易验证：
\begin{equation}
    \gamma^\mu p_\mu = \gamma^0 p^0 + \gamma^i p_i = \begin{pmatrix} 0 & p^0 I - \mathbf{p}\cdot\boldsymbol{\sigma} \\ p^0 I + \mathbf{p}\cdot\boldsymbol{\sigma} & 0 \end{pmatrix} = \begin{pmatrix} 0 & p_\mu \bar{\sigma}^\mu \\ p_\mu \sigma^\mu & 0 \end{pmatrix}
\end{equation}
...
\paragraph{分立对称性的矩阵算符}
在当前的经典容器层面，分立对称性表现为旋量空间中的特殊矩阵：

\textbf{1. 空间反演 (Parity, $\mathcal{P}$)}

空间反演使坐标 $\mathbf{x} \to -\mathbf{x}$，在狄拉克方程中翻转空间微商 $\partial_i \to -\partial_i$。我们需要一个矩阵能够同时实现"交换左右手分量"和"翻转空间 $\gamma^i$ 的符号"。根据 Clifford 代数 \eqref{eq:clifford}：
\begin{equation}
    \gamma^0 \gamma^i (\gamma^0)^{-1} = \gamma^0 \gamma^i \gamma^0 = -\gamma^i, \quad \gamma^0 \gamma^0 (\gamma^0)^{-1} = \gamma^0
\end{equation}
因此宇称矩阵正是 $\gamma^0$ 本身：
\begin{equation}
    \psi_P(t, \mathbf{x}) = \eta_P \gamma^0 \psi(t, -\mathbf{x})
\end{equation}

\textbf{2. 电荷共轭 (Charge Conjugation, $\mathcal{C}$)}
...
\begin{equation}
    C = i\gamma^2\gamma^0, \quad \psi_C(t, \mathbf{x}) = \eta_C C \bar{\psi}^T(t, \mathbf{x})
\end{equation}

\textbf{3. 时间反演 (Time Reversal, $\mathcal{T}$)}

时间反演是反线性算符，包含复共轭和时间坐标翻转 $t \to -t$：
\begin{equation}
    \psi_T(t, \mathbf{x}) = \eta_T B \psi^*(-t, \mathbf{x}), \quad \text{其中} \ B^{-1}\gamma^\mu B = (\gamma^\mu)^*
\end{equation}