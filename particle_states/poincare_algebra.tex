...
对比两边，我们得到 \(\mathscr{J}\) 和 \(\mathscr{P}\) 在Poincar\'e下的变换关系为：
    ...
    \end{align}
\end{subequations}
接下来，我们对(\ref{4.3a})和(\ref{4.3b})两式的变换算符 \({U}(\Lambda,\,a)
\rightarrow {U}(1+\omega,\epsilon)\)进行展开：\\
...
\begin{tcolorbox}[colback=white!5,colframe=black!55,title=Poincar\'e代数]
...
\begin{center}
    ...
    \vspace{0.5cm}
\end{center}
在$0+1$维量子场论中，一个特殊的角色由那些是守恒的，即与能量算符\(H = \mathscr{P}^{0}\)对易的算符扮演。
对方程(\refeq{4.6})的考察说明了这 是动量三矢
...
这些是不守恒的，这是为什么我们不使用K的本征值标记物理态的原因。在一个三维标记下，对
易关系(\refeq{4.6})可以重写为
