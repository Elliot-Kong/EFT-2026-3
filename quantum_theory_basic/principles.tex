...
\subsection{状态,线性叠加}
\begin{tcolorbox}[colback=white!5,colframe=black!55,title=线性的量子态]
...
\vspace{2ex}
    ...
    当系统涉及不同空间（如系统1和系统2）时，需区分\textbf{叠加}与\textbf{直积}：
    \begin{enumerate}
...
\begin{tcolorbox}[colback=white!5,colframe=black!55,title=态的幺正演化]
   ...
   \begin{equation}
    \begin{aligned}
        ...
        \underbrace{i\hbar\frac{dU(t,0)}{dt}U^{-1}(t,0)}_{H\footnote{$H$是系统的哈密顿量，作用到对应的态矢量上，便是系统的能量；也充分的说明了能量与时间的密切关系。}}\ket{\psi(t)}
        \\&=\left(i\hbar\frac{d}{dt}-H\right)\ket{\psi(t)}\footnote{这便是$Schrodinger$方程，方程中时间$t$具有独特的地位，所以方程中洛伦兹对称性不明显.
但“不明显对称”不等于“不对称”$Schrodinger$方程本身不需要被“相对论化；$Schrodinger$方程在我理解看来，是：幺正演化(态)的另一种形式.}
    \end{aligned}
   \end{equation}
\end{tcolorbox}
...
\normalcolor
由以上考虑,要对量子理论中的概率做出基本假设,不过首先应该提出“仪器”的性质；
\begin{tcolorbox}[colback=white!5,colframe=black!55,title=谱分解]
    一种“可能存在的理想仪器”,对应一组完备正交基$\left(\begin{aligned}
        &\ket{i}\\&\ket{\alpha}
    \end{aligned}\right)$.
    测量后,仪器示数为实数
    $\left(\begin{aligned}
    ...
    \end{aligned}\right)$可以理解为厄米算符\footnote{这里要论述一下self-adjoint operator以及
    可观测量是否和厄米算符等价的问题,实际上应该是observable=self-adjoint operator with spectral measure}
    \begin{equation}
            \mathscr{O}
            =\sum_{i} \lambda_i\ket{i}\bra{i}\
            =\int d\alpha\,\lambda_\alpha\ket{\alpha}\bra{\alpha}
    \end{equation}
    的本征态,本征值的集合.而$\mathscr{O}$便是这个理想仪器的可观测量.
\end{tcolorbox}

几点说明：\begin{enumerate}
...
\begin{tcolorbox}[colback=white!5,colframe=black!55,title=概率]
